<template>
  <ClientOnly>
    <div v-if="!showResendOtp" class="bg-red-500 text-white p-2 rounded-xl text-center" style="direction: ltr">
      <vue-countdown :time="120 * 1000" v-slot="{ minutes, seconds }" @end="onCountdownEnd"
    ...
    <button @click="resend" :disabled="loading" v-else class="flex justify-center gap-3 items-center bg-green-500 text-white p-2 rounded-xl text-center">
      ...
      <div role="status" v-if="loading">
        <svg aria-hidden="true" class="w-5 h-5  text-gray-200 animate-spin dark:text-gray-600 fill-red1"
